...
export class CachingService {
  ...
  async get<T>(key: string) {
    return new Promise<T>((resolve, reject) => {
      this._redisClient.get(key, (err, value) =>
        err ? reject(err) : resolve(JSON.parse(value) as T),
      );
    });
  }
  ...
  async set(key: string, value: unknown, exp?: number) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (exp) {
        this._redisClient.setex(key, exp, valStr, (err) =>
          err ? reject(err) : resolve(true),
        );
      } else {
        this._redisClient.set(key, valStr, (err) =>
          err ? reject(err) : resolve(true),
        );
      }
    });
